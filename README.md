...
3. Install deps and apply migrations:
   ...
   npm run db:migrate
   ```
4. (Optional) Seed conventions, artists, and applications:
   ```bash
...
The app is at <http://localhost:3000>. Default seed credentials are documented
in `scripts/seed-credentials.md` (gitignored — generated by the seed scripts).
