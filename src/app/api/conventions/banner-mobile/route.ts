...
export async function POST(request: NextRequest) {
  ...
  const storagePath = `conventions/${convention.id}/banner-mobile.webp`;

  try {
    const processed = await processImage(buffer);
    await storage.upload(storagePath, processed.data, "image/webp");

    const oldPath = convention.bannerMobilePath;
    ...
    console.error("Convention mobile banner upload failed:", error);
...
export async function DELETE() {
  ...
  if (!convention.bannerMobilePath) {
  ...
  await storage.delete(convention.bannerMobilePath).catch(() => {});
  return NextResponse.json({ ok: true });
}
